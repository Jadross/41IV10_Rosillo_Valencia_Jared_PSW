...
function examenF() {
  ...
  if (capitalIngresado == "") {
    alert("No se te olvide llenar todo");
    return false;
  } else {
    // Validacion extra
    if (lector.test(capitalIngresado)) {
      alert("Coloca datos que sirvena/validos");
      return false;
    } else {
      ...
      if (capitalIngresado > 0 && capitalIngresado < 9999999) {
        for (let i = 0; i < tiempoMeses; i++) {
          ...
          totalCuota = totalPago + totalInteres;
        }
        // Llenado de campos
        ...
        document.querySelector("#totalPago").textContent = totalPago.toFixed(2);
